...
void btmtk_platform_prop_register(struct btmtk_dev *bdev) {
	...
	if (mt_ptr != NULL) {
		if (strlen(mt_ptr) > PLATFORM_LEN + 2) {
			...
			BTMTK_INFO("%s: can not find platform id in dts root node", __func__);
			return;
		}
	} else {
...
int btmtk_connv3_sub_drv_init(struct btmtk_dev *bdev)
{
	...
	if(ret < 0)
		...
		BTMTK_INFO("%s: temp disable uart-irq-en", __func__);
	}
	...
	if (cif_dev->uart_irq_en) {
		...
		BTMTK_INFO("%s: not support uart wakeup irq", __func__);
...
void btmtk_dump_gpio_state(void)
{
	if (g_platform_prop == NULL) {
		BTMTK_INFO("%s: g_platform_prop not register yet", __func__);
		return;
	}
