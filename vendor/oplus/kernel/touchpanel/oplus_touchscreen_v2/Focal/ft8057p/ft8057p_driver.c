...
/*extern void fts_auto_write_result(struct auto_testdata *p_focal_testdata, int failed_count, bool black_screen_test);*/
...
void fts_black_screen_test(void *chip_data, char *msg)
{
	...
	struct fw_limit_data *fw_size = NULL;
	int ret = 0;
	size_t pos = 0;
	int error_count = 0;
	const struct firmware limit_fw = {
		.data = &fw_size->limit_data[0],
		.size = 1024 * 400,
	};
	...
	fts_test_ops = ts->com_test_data.chip_test_ops;
	if (!fts_test_ops) {
		TPD_INFO("Not support auto-test proc node\n");
		return;
	}

	p_focal_testdata->fp = vmalloc(1024 * 80 * 5);
	if (!p_focal_testdata->fp) {
		TPD_INFO("focal_testdata.fp malloc fail\n");
		return;
	}
	...
	if (!fts_test_ops->auto_test_preoperation) {
		...
		ret = fts_test_ops->auto_test_preoperation(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test1) {
		...
		ret = fts_test_ops->test1(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test2) {
		...
		ret = fts_test_ops->test2(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test3) {
		...
		ret = fts_test_ops->test3(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test4) {
		...
		ret = fts_test_ops->test4(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test5) {
		...
		ret = fts_test_ops->test5(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test6) {
		...
		ret = fts_test_ops->test6(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test7) {
		...
		ret = fts_test_ops->test7(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
	...
	if (!fts_test_ops->test8) {
		TPD_INFO("not support fts_test_ops->test8 callback\n");
	} else {
		ret = fts_test_ops->test8(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
			TPD_INFO("test8 failed\n");
			error_count++;
		}
	}

	if (!fts_test_ops->test9) {
		TPD_INFO("not support fts_test_ops->test9 callback\n");
	} else {
		ret = fts_test_ops->test9(NULL, ts->chip_data, p_focal_testdata, NULL);
		if (ret < 0) {
			TPD_INFO("test9 failed\n");
			error_count++;
		}
	}
	...
	if (!fts_test_ops->auto_test_endoperation) {
		...
		ret = fts_test_ops->auto_test_endoperation(NULL, ts->chip_data, p_focal_testdata, NULL);
	...
	/*fts_auto_write_result(p_focal_testdata, error_count, 1);*/
	snprintf(msg, 256, "test result:%s,%d\n", !error_count ? "PASS" : "FAIL", error_count);
...
static struct focal_auto_test_operations ft8057p_test_ops = {
	...
	.test1 = ft8057p_short_test,
	.test2 = ft8057p_open_test,
	.test3 = ft8057p_cb_test,
	.test4 = ft8057p_rawdata_autotest,
	.test5 = ft8057p_uniformity_autotest,
	.test6 = ft8057p_lcd_noise_test,
