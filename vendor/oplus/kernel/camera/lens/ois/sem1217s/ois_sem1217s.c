...
unsigned char ois_status(struct i2c_client *client)
{
    int i;
    ...
    for (i = 0; i < 50; i++) {
        do {
            ret = adaptor_i2c_rd_p8(client, client->addr, SEM1217S_REG_OIS_STS, &data, 1);
            if (ret >= 0) {
                break;
            } else {
            ...
            if (ret == -110) {
                data = 0;
                LOG_ERR("check OIS_STATE_READY status timeout, break");
                return data;
            }
            retry--;
        } while (retry > 0);
        ...
        if (ret < 0) {
            LOG_ERR("check OIS_STATE_READY status failed, i2c error, retry[%d]", retry);
            break;
        }

        if ((data & SEM1217S_STATE_READY) == 1) {
            break;
        }
    ...
    if (data != SEM1217S_STATE_READY) {
        OIS_LOG_IF(dbg, "OIS SEM1217S_STATE_READY status [%d]", data);
    }
...
static int reg_monitor_kthread(void *arg) {
    ...
    adaptor_i2c_rd_u16(client, client->addr, SEM1217S_REG_GX_GAIN, &gain_x);
    adaptor_i2c_rd_u16(client, client->addr, SEM1217S_REG_GY_GAIN, &gain_y);
...
static int sem1217s_init(struct sem1217s_device *sem1217s)
{
    ...
    ois_ret = ois_status(client);
    if ((ois_ret & 0x01) != 1) {
        LOG_ERR("ois device not ready. ois_ret[%u] return", ois_ret);
...
static int sem1217s_sample(struct hf_device *hfdev)
{
    ...
    for (i = 0; i < fifo_count; i++) {
        ...
        for (j = 0; j < GROUPS; j++) {
            event.word[2 * j] = 0;
            event.word[2 * j + 1] = 0;
            event.word[2 * j + 2] = sem1217s_hall_convert(((read_buff[i * 4] << 8) + read_buff[i * 4 + 1]));
            event.word[2 * j + 3] = sem1217s_hall_convert(((read_buff[i * 4 + 2] << 8) + read_buff[i * 4 + 3]));
            event.word[2 * j + 4] = event.word[2 * j + 2];
            event.word[2 * j + 5] = event.word[2 * j + 3];
            OIS_LOG_IF(buffer_dump, "eisbuffer X event.word[%d] index[%d] gyro[%d] target[%d] hall[%d]",2 * j,2 * i * GROUPS + 2 * j, event.word[2 * j], event.word[2 * j + 2], event.word[2 * j + 4]);
