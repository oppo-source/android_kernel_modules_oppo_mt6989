...
void oplus_display_apollo_init_para(struct drm_crtc *crtc)
{
	...
	mtk_crtc->oplus_apollo_br->last_cur_vsync = 0;
	mtk_crtc->oplus_apollo_br->last_limit_superior_ns = 0;
	mtk_crtc->oplus_apollo_br->last_limit_inferior_ns = 0;
	mtk_crtc->oplus_apollo_br->last_transfer_time_us = 0;
}
	...
	unsigned int panel_ext_param, unsigned int cfg_flag)
{
	...
	if (mtk_crtc->panel_ext->params->dyn_fps.vact_timing_fps != 0) {
		cur_vsync = 1000000000 / mtk_crtc->panel_ext->params->dyn_fps.vact_timing_fps;
	} else {
		DDPPR_ERR("%s:%d fps get failed! \n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (mtk_crtc->oplus_apollo_br->last_cur_vsync == 0 && cur_vsync != 0) {
		mtk_crtc->oplus_apollo_br->last_cur_vsync = cur_vsync;
		mtk_crtc->oplus_apollo_br->last_limit_superior_ns = limit_superior_ns;
		mtk_crtc->oplus_apollo_br->last_limit_inferior_ns = limit_inferior_ns;
		mtk_crtc->oplus_apollo_br->last_transfer_time_us = transfer_time_us;
	}

	if (strcmp(mtk_crtc->panel_ext->params->vendor, "A0004") == 0) {
		if (mtk_crtc->oplus_apollo_br->last_cur_vsync == APOLLO_CUR_VSYNC_60) {
	...
	if (mtk_crtc->oplus_apollo_br->oplus_backlight_need_sync || adfr_fps_need_sync) {
		...
		if (mtk_crtc->oplus_apollo_br->oplus_refresh_rate_switching) {
			usleep_range(mtk_crtc->oplus_apollo_br->last_cur_vsync/1000, mtk_crtc->oplus_apollo_br->last_cur_vsync/1000 + 100);
		}

		time_gap_ns = ktime_get() > mtk_crtc->oplus_apollo_br->oplus_te_tag_ns ? ktime_get() - mtk_crtc->oplus_apollo_br->oplus_te_tag_ns : 0;
		if (time_gap_ns >= 0 && time_gap_ns <= mtk_crtc->oplus_apollo_br->last_cur_vsync) {
			if (time_gap_ns < mtk_crtc->oplus_apollo_br->last_limit_superior_ns) {
				if ((mtk_crtc->oplus_apollo_br->oplus_te_tag_ns > mutex_sof_ns
						&& mtk_crtc->oplus_apollo_br->oplus_te_tag_ns - mutex_sof_ns < mtk_crtc->oplus_apollo_br->last_cur_vsync / 2)
					|| (mutex_sof_ns > mtk_crtc->oplus_apollo_br->oplus_te_tag_ns
						&& mutex_sof_ns - mtk_crtc->oplus_apollo_br->oplus_te_tag_ns < mtk_crtc->oplus_apollo_br->last_cur_vsync / 2)) {
					delay_us = mtk_crtc->panel_ext->params->dyn_fps.apollo_limit_superior_us - mtk_crtc->oplus_apollo_br->last_transfer_time_us;
				} else {
					delay_us = (mtk_crtc->oplus_apollo_br->last_limit_superior_ns - time_gap_ns) / 1000;
				}
				if (delay_us > 0) {
			...
			} else if (time_gap_ns > mtk_crtc->oplus_apollo_br->last_limit_inferior_ns) {
				delay_us = (mtk_crtc->oplus_apollo_br->last_cur_vsync - time_gap_ns) / 1000 + mtk_crtc->panel_ext->params->dyn_fps.apollo_limit_superior_us;
				if (delay_us > 0) {
