...
static int oplus_display_panel_set_pwm_pul(void *dsi, dcs_write_gce_pack cb, void *handle, unsigned int mode)
{
	...
	switch (mode) {
		...
		case PWM_SWITCH_18TO1: {
			if (pwm_params->lhbm_wait_te) {
				pr_info("pwm_turbo onepulse dsi_pwm_switch 18to1 wait te\n");
				oplus_display_panel_wait_te(1);
				pwm_params->lhbm_wait_te = false;
			}
		...
		case PWM_SWITCH_1TO18: {
			if (pwm_params->lhbm_wait_te) {
				pr_info("pwm_turbo onepulse dsi_pwm_switch 1to18 wait te\n");
				oplus_display_panel_wait_te(1);
				pwm_params->lhbm_wait_te = false;
			}
