			      ...
			      size_t pgcount, int prot, gfp_t gfp, size_t *mapped)
{
	...
	while (ret == -ENOMEM && (gfp & GFP_ATOMIC) != 0 && retry_count < 8) {
		...
		dev_info(smmu->dev, "%s, retry map alloc memory %d gfp:0x%x, ret:%d\n",
			 __func__, retry_count + 1, gfp_flags, ret);
	...
	if (ret)
		dev_info(smmu->dev,
			 "%s, %d:%d iova:0x%lx pgsize:0x%zx pgcount:0x%zx prot:%d gfp:0x%x\n",
			 __func__, ret, retry_count, iova, pgsize, pgcount, prot, gfp);
