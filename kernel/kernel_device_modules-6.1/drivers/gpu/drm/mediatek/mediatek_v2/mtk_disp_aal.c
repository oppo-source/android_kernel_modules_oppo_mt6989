	...
	const int block_x, const int block_y, const int dre_blk_x_num, int check)
{
	...
	do {
		...
		if (check && value != aal_data->primary_data->dre30_gain.dre30_gain[block_offset]) {
			DDPAEE(CURVE_AEE_STR, __func__, __LINE__,
			       dre_blk_x_num, block_x, block_y, block_offset,
		...
		for (blk_x = 0; blk_x < dre_blk_x_num; blk_x++) {
			...
			if (bit_shift >= 4) {
				...
				if (check &&
					...
					DDPAEE(CURVE_AEE_STR, __func__, __LINE__,
					       dre_blk_x_num, blk_x, blk_y,
					       block_offset, write_value,
...
static void disp_aal_dre3_check_reset_to_linear(struct mtk_ddp_comp *comp)
{
	...
	for (blk_y = 0; blk_y < dre_blk_y_num; blk_y++) {
		for (blk_x = 0; blk_x < dre_blk_x_num; blk_x++) {
			...
			if (!write_block(aal_data, dre3_gain, blk_x, blk_y, dre_blk_x_num, 0))
				goto _return;
		}
	}
	/* write each block dre curve last point */
	if(!write_curve16(aal_data, dre3_gain, dre_blk_x_num, dre_blk_y_num, 0))
		goto _return;
_return:
	spin_unlock_irqrestore(&aal_data->primary_data->dre3_gain_lock, flags);
}
