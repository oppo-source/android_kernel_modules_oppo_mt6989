...
static int vdisp_level_set_vcp(const enum mtk_dpc_subsys subsys, const u8 level)
{
	...
	ret = readl_poll_timeout_atomic(g_priv->vdisp_dvfsrc_check, value,
					(value & 0x3) == 0, 1, 500);
	if (ret < 0)
...
static int dpc_check_disp_vcore(void)
{
	if (0 == (readl(g_priv->spm_base + SPM_PWR_STATUS_MSB) & BIT(3))) {
		DPCDUMP("disp vcore is not power on");
		return -1;
	}
	return 0;
}
