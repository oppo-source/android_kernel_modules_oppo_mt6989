	...
	char *aed_str, int db_opt)
{
	...
	//#ifdef OPLUS_FEATURE_MODEM_MINIDUMP
	aed_str_for_hash = getTargetString(aed_str);
	if( aed_str_for_hash != NULL ) {
		hashId = BKDRHash(aed_str_for_hash, strlen(aed_str_for_hash), 0); /* calculate hashid */
	}
	else {
...
char* getTargetString(char *aed_str) {
	char *aed_str_for_hash = aed_str;
	int cursor_idx = 0;
