...
$(KERNEL_ZIMAGE_OUT): $(KERNEL_MAKE_DEPENDENCIES)
	...
	$(hide) sed -i 's/platform/$(COMPILE_PLATFORM)/g' kernel/oplus/bazel/oplus_platform.bzl
	$(hide) cd kernel && export BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN=1 && tools/bazel --output_root=$(abspath $(PRIVATE_BAZEL_BUILD_OUT)) --output_base=$(abspath $(PRIVATE_BAZEL_BUILD_OUT))/bazel/output_user_root/output_base build $(PRIVATE_BAZEL_BUILD_FLAG) $(PGO_KBUILD_ARG) $(PRIVATE_BAZEL_BUILD_GOAL)
	$(hide) cd kernel && export BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN=1 && tools/bazel --output_root=$(abspath $(PRIVATE_BAZEL_BUILD_OUT)) --output_base=$(abspath $(PRIVATE_BAZEL_BUILD_OUT))/bazel/output_user_root/output_base run $(PRIVATE_BAZEL_BUILD_FLAG) $(PGO_KBUILD_ARG) --nokmi_symbol_list_violations_check $(PRIVATE_BAZEL_DIST_GOAL) -- --dist_dir=$(abspath $(PRIVATE_BAZEL_DIST_OUT))
ifeq (true,$(strip $(KERNEL_CHECK_ABI)))
	...
	$(hide) sed -i 's/$(COMPILE_PLATFORM)/platform/g' kernel/oplus/bazel/oplus_platform.bzl
	$(hide) touch kernel/kernel_device_modules-6.1/BUILD.bazel
endif
